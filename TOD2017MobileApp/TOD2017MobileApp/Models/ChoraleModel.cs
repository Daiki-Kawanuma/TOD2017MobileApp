...
namespace TOD2017MobileApp.Models
{
	public class ChoraleModel
	{
		...
		public static ChoraleModel CreateChoraleModel(SemanticLink semanticLink)
		{
			...
			{
				ClassNumber = MathUtil.CalculateClassNumber((IList)data),
				MinLostEnegry = data.Min(d => d.LostEnergy),
				MaxLostEnergy = data.Max(d => d.LostEnergy),
				ClassWidthEnergy = (data.Max(d => d.LostEnergy) - data.Min(d => d.LostEnergy)) / MathUtil.CalculateClassNumber((IList)data),
				MinTransitTime = data.Min(d => d.TransitTime),
				MaxTransitTime = data.Max(d => d.TransitTime),
				ClassWidthTransitTime = (float)(data.Max(d => d.TransitTime) - data.Min(d => d.TransitTime)) / MathUtil.CalculateClassNumber((IList)data)
			};
			model.SetData(data);
